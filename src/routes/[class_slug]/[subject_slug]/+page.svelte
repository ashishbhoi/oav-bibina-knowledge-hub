...
<svelte:head>
  <title
    >{data.subject.name} - {data.class.name} | OAV Bibina Knowledge Hub</title
  >
  <meta
    name="description"
    content="Download notes and study materials for {data.subject.name} in {data
      .class.name}. Access PDFs, documents, and other academic resources."
  />
</svelte:head>
  ...
  <header class="bg-white shadow-sm border-b">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
      <nav class="flex items-center space-x-2 text-sm text-gray-500 mb-2">
        ...
        <span>/</span>
        <a class="hover:text-blue-600" href="/{createSlug(data.class.name)}"
          >{data.class.name}</a
        >
        <span>/</span>
    ...
    {:else}
      <div class="text-center py-12">
        ...
        <p class="text-gray-500">
          Study materials for this subject will appear here once they are
          uploaded.
        </p>
      </div>
    {/if}
