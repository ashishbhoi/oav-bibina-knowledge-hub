<script lang="ts">
    interface Props {
        data: {
            ...
            groupedNotes: Record<string, Array<{
                id: number;
                display_name: string;
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                        ...
                        >
                            <div class="flex items-center justify-between">
                                <div class="flex items-center space-x-3">
                                    ...
                                    <div>
                                        <h3 class="text-lg font-semibold text-gray-900">{fileType}</h3>
                                        <p class="text-sm text-gray-500">{notes.length}
                                            file{notes.length !== 1 ? 's' : ''}</p>
                                    </div>
                                </div>
                                <svg
                                        class="w-5 h-5 text-gray-400 transition-transform duration-200 {expandedSections[fileType] ? 'rotate-180' : ''}"
                                        fill="none"
                                        stroke="currentColor"
                                        viewBox="0 0 24 24"
                                >
                                        ...
                                        {#each notes as note}
                                            ...
                                            >
                                                <div class="flex items-center space-x-3">
                                                    ...
                                                    <div>
                                                        <p class="font-medium text-gray-900">{note.display_name}</p>
                                                        <p class="text-sm text-gray-500">
                                                            Uploaded: {new Date(note.uploaded_at).toLocaleDateString()}</p>
                                                    </div>
                                                </div>
        ...
        {:else}
            <div class="text-center py-12">
                ...
                <p class="text-gray-500">Study materials for this subject will appear here once they are uploaded.</p>
            </div>
        {/if}
    </main>
