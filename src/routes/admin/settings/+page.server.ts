...
const fileTypeSchema = z.object({
	name: z.string().min(1, 'File type name is required').max(50, 'File type name too long'),
});
...
export const actions: Actions = {
	...
	createSubject: async ({ request, platform }) => {
		...
		try {
			...
			await createSubject(platform.env.DB, validation.data.name, validation.data.class_id);
	...
	updateSubject: async ({ request, platform }) => {
		...
		try {
			...
			await updateSubject(platform.env.DB, data.id, validation.data.name, validation.data.class_id);
	...
	updateAdmin: async ({ request, platform, locals }) => {
		...
		try {
			...
			const admin = await getAdminByUsername(platform.env.DB, locals.user.username);
			if (
				!admin ||
				!(await verifyPassword(validation.data.current_password, admin.password_hash))
			) {
				return fail(400, {
